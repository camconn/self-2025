...
pub export fn counter(arg_file: [*c]u8) void {
    ...
    _ = &fp;
    if (!(fp != null)) {
        errf(@constCast("cannot open file"));
    }
    ccount = blk: {
        const tmp = blk_1: {
...
pub fn main() !u8 {
    ...
    _ = args.skip(); // skip the name of the program

    var got_arg: bool = false;
    while (args.next()) |filename| {
        got_arg = true;
        counter(@constCast(filename.ptr));
    }

    if (!got_arg) {
        errf(@constCast("usage: wc FILE [FILE...]"));
        return 1;
    }

    if (got_arg) {
        report(@constCast("total"), total_ccount, total_wcount, total_lcount);
    }
    return 0;
}
